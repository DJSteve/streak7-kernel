...
struct wm8903_wired_jack_conf {
	struct tegra_wired_jack_conf _wired_jack_conf;
	int amp_reg_enabled;
};
...
extern int luna_audio_debug_mask;
#define LUNA_AUDIO_FUNCTION_TRACE	(1 << 0)
#define LUNA_AUDIO_ERR				(1 << 1)
#define LUNA_AUDIO_DAPM_DEBUG		(1 << 2)
#define LUNA_AUDIO_HEADSET_DEBUG	(1 << 3)
#define LUNA_AUDIO_DEBUG			(1 << 4)

#define ASOC_FUNCTION(fmt, arg...)	\
		do { \
		   if (luna_audio_debug_mask & LUNA_AUDIO_FUNCTION_TRACE) \
		   printk(KERN_INFO "[ASOC-FTRACE]+%s " fmt "\n" , __FUNCTION__ , ## arg); \
		} while(0);

#define ASOC_INFO(fmt, arg...)		\
		do { if (luna_audio_debug_mask & LUNA_AUDIO_FUNCTION_TRACE) \
		   printk(KERN_INFO "[ASOC-INFO]%s " fmt "\n" , __FUNCTION__ , ## arg); \
		} while(0);

#define ASOC_DBG(fmt, arg...)  		\
		do { if (luna_audio_debug_mask & LUNA_AUDIO_DEBUG) \
		   printk(KERN_DEBUG "[ASOC-DBG]%s " fmt "\n" , __FUNCTION__ , ## arg); \
		} while(0);

#define ASOC_DAPM_DBG(fmt, arg...)  		\
		do { if (luna_audio_debug_mask & LUNA_AUDIO_DAPM_DEBUG) \
		   printk(KERN_INFO "[ASOC-DAPM]%s " fmt "\n" , __FUNCTION__ , ## arg); \
		} while(0);

#define ASOC_HEADSET_DBG(fmt, arg...)  		\
		do { if (luna_audio_debug_mask & LUNA_AUDIO_HEADSET_DEBUG) \
		   printk(KERN_INFO "[ASOC-HEADSET]%s " fmt "\n" , __FUNCTION__ , ## arg); \
		} while(0);

#define ASOC_ERR(fmt, arg...)  		\
		do { if (luna_audio_debug_mask & LUNA_AUDIO_ERR) \
		   printk(KERN_ERR  "[ASOC-ERR]%s " fmt "\n" , __FUNCTION__ , ## arg); \
		} while(0);
...
#define LUNA_EXTERNAL_MIC_SETTING_L2L1 (0x1<<B06_IN_CM_ENA) \
                    | (0x0<<B04_IP_SEL_N)   \
                    | (0x1<<B02_IP_SEL_P)   \
                    | (0x2<<B00_MODE);     

#define LUNA_INTERNAL_MIC_SETTING_R1R2 (0x1<<B06_IN_CM_ENA) \
                    | (0x1<<B04_IP_SEL_N)   \
                    | (0x0<<B02_IP_SEL_P)   \
                    | (0x2<<B00_MODE);     

#define LUNA_SPEAKER_CORRECT_DACL_PHASE_EVT3_1() (((system_rev) > EVT3_1_B)&&((system_rev) != HWID_UNKNOWN)) 
