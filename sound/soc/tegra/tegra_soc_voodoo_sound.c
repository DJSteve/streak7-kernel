 ...
 *
 * Copyright (c) 2010-2011, François Simond aka supercurio on tiwtter & XDA
 *
...
static ssize_t headphone_amplifier_level_show(struct device *dev,
					      ...
					      char *buf)
{
	return sprintf(buf, "%u\n", audio_params[EP101].analog_headset_volume);
}
					       ...
					       const char *buf, size_t size)
{
	...
	if (sscanf(buf, "%hu", &vol) == 1) {
		...
		audio_params[EP101].analog_headset_volume = vol;
