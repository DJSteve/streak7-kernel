...
int wakeup_irq = 0;
extern unsigned long _wake_pin;
#endif
...
unsigned long wake_time_arr[32][2];
u64 total_deepsleep_time;
u64 deepsleep_time;

inline int tegra_get_wake_status(void) {
	if (_wake_pin < 32)
		return _wake_pin;
	else
		return -1;
...
static int tegra_suspend_enter(suspend_state_t state)
{
	...
	u64 wake_secs;
	u32 ms;
	static u64 rtc_before;
	static u64 rtc_after;
    ...
    if (rtc_after != 0)
    {
		int wake_source = tegra_get_wake_status();
        wake_secs = rtc_before - rtc_after;
		printk("%s %d => wake_secs %lld\n", __func__, wake_source, wake_secs);
        if (wake_secs != -1);
        {
            wake_time_arr[wake_source][0] += wake_secs;
            if (wake_time_arr[wake_source][1] < wake_secs)
            {
                wake_time_arr[wake_source][1] = wake_secs;
            }
        }
    }
	...
	printk(KERN_ERR "<POWER>resumeing at RTC %llu\n",rtc_after);
	real_suspend=1;

    deepsleep_time += secs;
    total_deepsleep_time += secs;
	...
	printk(KERN_ERR "<POWER>resume1[CPU leave suspend]take %llu ms from RTC\n",tegra_rtc_read_ms()-rtc_after);
	return 0;
}
