...
static void clock_tree_show_one(struct seq_file *s, struct clk *c, int level)
{
	...
	if ((c->reg >= 0x80) && (c->reg <= 0xf0))
	{
		sprintf(reg_value, " -- (%08x)", clk_readl(c->reg));
	}

	seq_printf(s, "%*s%c%c%-*s %-6s %-3d %-8s %-10lu%s\n",
		level * 3 + 1, "",
		c->rate > c->max_rate ? '!' : ' ',
		!c->set ? '*' : ' ',
		30 - level * 3, c->name,
		state, c->refcnt, div, clk_get_rate_all_locked(c), reg_value);
