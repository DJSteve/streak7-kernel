...
static ssize_t sysfs_OemBoardId_show(struct kobject *kobj,
        struct kobj_attribute *attr, char *buf);

static ssize_t sysfs_sysrev_show(struct kobject *kobj,
        struct kobj_attribute *attr, char *buf);

static struct kobj_attribute nvfuse_OemBoardId_attr =
    __ATTR(BoardId, 0440, sysfs_OemBoardId_show, 0);

static struct kobj_attribute nvfuse_SystemRev_attr =
    __ATTR(SystemRev, 0440, sysfs_sysrev_show, 0);
...
char OemBoardId[BOARD_ID_LEN+1];
static int __init OemBoardId_setup(char *options)
{
    int i;
    if (!options || !*options)
        return 0;

    for (i=0; (i<BOARD_ID_LEN) && (options[i]) && (options[i]!=' '); i++)
        OemBoardId[i] = options[i];
    OemBoardId[i] = 0;

    return 0;
}

__setup("OemBoardId=", OemBoardId_setup);

int IsWIFIVersion(void)
{
	if(strncmp(OemBoardId,"EVT2-WIFI",strlen("EVT2-WIFI")) == 0 ||
	strncmp(OemBoardId,"DVT1-WIFI",strlen("DVT1-WIFI")) == 0)
	{
		return 1;
	}
	else
...
int IsA02Version(void)
{
	char *pch; 
	
	pch = strstr(OemBoardId,"A02");
	if(pch != NULL)
	{
		return 1;
	}
...
static int __init tegra_fuse_program_init(void)
{
    ...
    nvfuse_SystemRev_attr.attr.mode = 0640;
    nvfuse_OemBoardId_attr.attr.mode = 0644;
    CHK_ERR(sysfs_create_file(fuse_kobj, &nvfuse_SystemRev_attr.attr));
    CHK_ERR(sysfs_create_file(fuse_kobj, &nvfuse_OemBoardId_attr.attr));
