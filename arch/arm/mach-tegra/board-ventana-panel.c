...
#define ventana_pnl_pwr_enb	TEGRA_GPIO_PS1
#define ventana_bl_enb		TEGRA_GPIO_PR7
...
static struct regulator *ventana_gpio_hdmi_reg = NULL;
...
static int ventana_backlight_init(struct device *dev) {
	...
	gpio_export(ventana_bl_enb, false);
...
static int ventana_backlight_notify(struct device *unused, int brightness)
{
  ...
  static int state_old = 2;
  int state = !!brightness;
  if(state_old != state)
  {
    state_old = state;
    printk("[BKL]Backlight %s (%3d)\n",state?"ON ":"OFF",brightness);
  }
  

	
	
	if (old_brightness == 0 && brightness)
		msleep(200);
...
static struct tegra_dc_out_pin ventana_disp1_out_pins[] = {
	{
...
static struct tegra_dc_out ventana_disp1_out = {
	...
	.out_pins	= ventana_disp1_out_pins,
	.n_out_pins	= ARRAY_SIZE(ventana_disp1_out_pins),
...
int __init ventana_panel_init(void)
{
	...
	gpio_export(ventana_pnl_pwr_enb, false);
	...
	*/
	...
	ventana_panel_early_suspender.level = EARLY_SUSPEND_LEVEL_STOP_DRAWING - 1;
...
static int __init ventana_hdmi_enb_init(void)
{
	if (!ventana_gpio_hdmi_reg) {
		ventana_gpio_hdmi_reg = regulator_get(NULL, "gpio_vdd_hdmi"); 
		if (IS_ERR_OR_NULL(ventana_gpio_hdmi_reg)) {
			pr_err("hdmi: couldn't get regulator gpio_vdd_hdmi\n");
			ventana_gpio_hdmi_reg = NULL;
			return PTR_ERR(ventana_gpio_hdmi_reg);
		}
	}
	regulator_enable(ventana_gpio_hdmi_reg);
	pr_err("## hdmi_enb_init ##\n");

	return 0;
}
late_initcall(ventana_hdmi_enb_init);
