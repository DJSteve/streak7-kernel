...
static int ventana_wifi_get_mac_addr(unsigned char *buf);

unsigned char wifi_mac[wifi_mac_str_length] = {0};
unsigned char mac_set_flag = 0;
...
static struct wifi_platform_data ventana_wifi_control = {
	...
	.get_mac_addr	= ventana_wifi_get_mac_addr,
};

static struct resource ventana_wifi_resources[] = {
	{
		.name   =   "bcm4329_wlan_irq",
		.start  =   VENTANA_WLAN_IRQ,
		.end    =   VENTANA_WLAN_IRQ,
...
static int ventana_wifi_get_mac_addr(unsigned char *buf)
{
	unsigned char mac_addr[wifi_mac_size] = {0}; 
	...
	if (mac_set_flag) {
		ptr = wifi_mac;
		for (i = 0 ; i < wifi_mac_size ; i++) {
			memcpy(tmp, ptr, 2);
			if (tmp == '0') {
				mac_addr[i] = 0x00;
			}
			else {
				sscanf(tmp, "%02x", &mac_num);
				mac_addr[i] = (unsigned char)mac_num;
			}
			ptr += 2;
			memset(tmp, '\0', sizeof(tmp));
		}
		memcpy(buf, mac_addr, wifi_mac_size);
	} else {
		pr_warning("Use Defaulf Mac Address\n");
	}
	return 0;
}
...
#ifdef CONFIG_WLAN_ALLOC_STATIC_MEM 
    printk("Entry WLAN ALLOC STATIC MEM\n");
    addr_4329_dhd_prot  = kzalloc(ADDR_4329_DHD_PORT_LEN, GFP_NOWAIT);
    addr_4329_bus_rxbuf = kzalloc(ADDR_4329_BUS_RXBUF_LEN, GFP_NOWAIT);
    addr_4329_bus_databuf = kzalloc(ADDR_4329_BUS_DATABUF_LEN, GFP_NOWAIT);
