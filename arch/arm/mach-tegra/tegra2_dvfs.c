...
static const int core_millivolts[MAX_DVFS_FREQS] =
	{950, 1000, 1100, 1200, 1225, 1275, 1300, 1600};
static const int cpu_millivolts[MAX_DVFS_FREQS] =
#ifdef CONFIG_TEGRA_OC
	{750, 775, 800, 825, 850, 875, 900, 925, 950, 975, 1000, 1175, 1275, 1350, 1450, 1450};
#else
	{750, 775, 800, 825, 850, 875, 900, 925, 950, 975, 1000, 1025, 1050, 1100};
#endif
...
static struct dvfs dvfs_init[] = {
...
#ifdef CONFIG_TEGRA_OC
	CPU_DVFS("cpu", 0, 0, MHZ, 314, 314, 314, 456, 456, 456,  608,  608,  608,  760,  817,  817,  912,  1000, 1200, 1400, 1504, 1600, 1700),
 	CPU_DVFS("cpu", 0, 1, MHZ, 314, 314, 314, 456, 456, 456,  618,  618,  618,  770,  827,  827,  922,  1000, 1200, 1400, 1504, 1600, 1700),
 	CPU_DVFS("cpu", 0, 2, MHZ, 494, 494, 494, 675, 675, 817,  817,  922,  922,  1000, 1200, 1400, 1504, 1600, 1700),
 	CPU_DVFS("cpu", 0, 3, MHZ, 730, 760, 845, 845, 940, 1000, 1200, 1400, 1504, 1600, 1700),

 	CPU_DVFS("cpu", 1, 0, MHZ, 380, 380, 503, 503, 655, 655,  798,  798,  902,  902,  960,  1000, 1200, 1400, 1504, 1600, 1700),
 	CPU_DVFS("cpu", 1, 1, MHZ, 389, 389, 503, 503, 655, 760,  798,  798,  950,  950,  1000, 1200, 1400, 1504, 1600, 1700),
 	CPU_DVFS("cpu", 1, 2, MHZ, 598, 598, 750, 750, 893, 893,  1000, 1200, 1400, 1504, 1600, 1700),
 	CPU_DVFS("cpu", 1, 3, MHZ, 730, 760, 845, 845, 940, 1000, 1200, 1400, 1504, 1600, 1700),
#else
	 ...
	 */
	CORE_DVFS("disp1",   -1, 0, KHZ, 158000, 158000, 190000, 190000, 200000, 200000, 200000),
	...
	CORE_DVFS("epp",     -1, 1, KHZ, 133000, 171000, 247000, 450000, 450000, 450000, 450000),
	CORE_DVFS("2d",      -1, 1, KHZ, 133000, 171000, 247000, 450000, 450000, 450000, 450000),

	CORE_DVFS("3d",       0, 1, KHZ, 114000, 161500, 247000, 304000, 450000, 450000, 450000),
	CORE_DVFS("3d",       1, 1, KHZ, 161500, 209000, 285000, 333500, 450000, 450000, 450000),
	CORE_DVFS("3d",       2, 1, KHZ, 218500, 256500, 323000, 380000, 450000, 450000, 450000),
	CORE_DVFS("3d",       3, 1, KHZ, 247000, 285000, 351500, 400000, 450000, 450000, 450000),

	CORE_DVFS("mpe",      0, 1, KHZ, 104500, 152000, 228000, 300000, 450000, 450000, 450000),
	CORE_DVFS("mpe",      1, 1, KHZ, 142500, 190000, 275500, 300000, 450000, 450000, 450000),
	CORE_DVFS("mpe",      2, 1, KHZ, 190000, 237500, 300000, 300000, 450000, 450000, 450000),
	CORE_DVFS("mpe",      3, 1, KHZ, 228000, 266000, 300000, 300000, 450000, 450000, 450000),

	CORE_DVFS("vi",      -1, 1, KHZ, 85000,  100000, 150000, 150000, 150000, 150000, 150000),

	CORE_DVFS("sclk",     0, 1, KHZ, 95000,  133000, 190000, 222500, 315000, 315000, 315000),
	CORE_DVFS("sclk",     1, 1, KHZ, 123500, 159500, 207000, 240000, 315000, 315000, 315000),
	CORE_DVFS("sclk",     2, 1, KHZ, 152000, 180500, 229500, 260000, 315000, 315000, 315000),
	CORE_DVFS("sclk",     3, 1, KHZ, 171000, 218500, 256500, 292500, 315500, 315000, 315000),

	CORE_DVFS("vde",      0, 1, KHZ, 95000,  123500, 209000, 275500, 315000, 315000, 315000),
	CORE_DVFS("vde",      1, 1, KHZ, 123500, 152000, 237500, 300000, 315000, 315000, 315000),
	CORE_DVFS("vde",      2, 1, KHZ, 152000, 209000, 285000, 300000, 315000, 315000, 315000),
	CORE_DVFS("vde",      3, 1, KHZ, 171000, 218500, 300000, 300000, 315000, 315000, 315000),
	/* What is this? */
