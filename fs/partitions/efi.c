...
static u64 force_mbr_offset_lba = 0;	
static int __init
...
static int __init force_mbr_offset_lba_fn(char *str)
{
	force_mbr_offset_lba = simple_strtoull(str, NULL, 0);
	printk(KERN_WARNING "%s: force_mbr_offset_lba=%lld\n", __func__, force_mbr_offset_lba);
	return 1;
}
__setup("mbr_offset_lba=", force_mbr_offset_lba_fn);
			  ...
			  gpt_entry **ptes)
{
	...
	lastlba = last_lba(state->bdev);
        if (!force_gpt) {
                ...
                if (legacymbr) {
                        ...
                        mbr_lba = (force_mbr_offset_lba) ? force_mbr_offset_lba : 0;    
                        read_lba(state, mbr_lba, (u8 *) legacymbr,
				 sizeof (*legacymbr));
	...
	gpt_primary_lba = (force_mbr_offset_lba) ? (force_mbr_offset_lba+1) : GPT_PRIMARY_PARTITION_TABLE_LBA;	
	good_pgpt = is_gpt_valid(state, gpt_primary_lba,
				 &pgpt, &pptes);
        if (good_pgpt)
