...
extern struct parsed_partitions *global_state;
#endif
				...
				sector_t start, sector_t len, int flags)
{
	...
	if (flags & ADDPART_FLAG_QISDA_READONLY) {
		printk(KERN_WARNING "Qisda backdoor: set p%d to read-only\n", partno);
		p->policy = 1;
		flags &= ~ADDPART_FLAG_QISDA_READONLY;
	}
