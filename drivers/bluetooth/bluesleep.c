...
#define BT_DBG(fmt, arg...) \
		do { \
		   printk(KERN_ERR  "[BLUESLEEP]%s " fmt "\n" , __FUNCTION__ , ## arg); \
		} while(0);
#endif
/*
 * Defines
...
extern int bcom4329_get_wakepin(void);
...
static int bluesleep_probe(struct platform_device *pdev)
{
...
#if 1
	ret = bcom4329_get_wakepin();
	if (ret)
	{
		BT_ERR("get bt_ext_wake gpio from bcom4329_get_wakepin\n");
		bsi->ext_wake = ret;
		bsi->has_ext_wake = 1;
	...
	{
		BT_ERR("couldn't gpio_request bt_ext_wake gpio\n");
		bsi->has_ext_wake = 0;
	}
#else
	res = platform_get_resource_byname(pdev, IORESOURCE_IO,
