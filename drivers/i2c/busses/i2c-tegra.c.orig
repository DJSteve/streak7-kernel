...
#define MSG2(format, arg...)  {printk(KERN_INFO "[I2C]" format "\n", ## arg);}
...
static int tegra_i2c_probe(struct platform_device *pdev)
{
  ...
  MSG2("%s+, id=%d, nr=%d",__func__,pdev->id,
    ((struct tegra_i2c_platform_data *)pdev->dev.platform_data)->adapter_nr);
...
static int tegra_i2c_suspend_noirq(struct device *dev)
{
  ...
  if(((struct tegra_i2c_platform_data *)pdev->dev.platform_data)->adapter_nr == 4)
    MSG2("%s, (nr=%d)",__func__,((struct tegra_i2c_platform_data *)pdev->dev.platform_data)->adapter_nr);
