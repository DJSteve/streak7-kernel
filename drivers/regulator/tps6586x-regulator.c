...
static int gpio_3v3_capchg_inited = 0;  
				    ...
				    int min_uV, int max_uV)
{
  ...
  MSG2("%s,  %s %d~%d",__func__,rdev->desc->name,min_uV/1000,max_uV/1000);
...
static int ldo_test_mode = 0;
void ftm_test_mode_onOff(int onOff)
{
  if(onOff)
    ldo_test_mode = 1;
  else
...
int ftm_test_regulator_enable(struct regulator *rr)
{
  struct regulator_dev *rdev = rr->rdev;
	struct tps6586x_regulator *ri = rdev_get_drvdata(rdev);
	struct device *parent = to_tps6586x_dev(rdev);
  ...
  if(id == TPS6586X_ID_GPIO_2 && gpio_3v3_capchg_inited)
  {
    unsigned gpio = TEGRA_GPIO_PS7;
    int ret;
    uint8_t reg_val;
  	ret = tps6586x_read(parent, ri->enable_reg[0], &reg_val);
    if(ret) { MSG2("%s, read %02X, Fail = %d", __func__, ri->enable_reg[0], ret);  }
    
    if(!!(reg_val & (1 << ri->enable_bit[0])))  
    {
      MSG2("%s, 3V3_CAPCHG, Bypass", __func__);
    }
    else
    {
  ...
  else if(id == TPS6586X_ID_LED_PWM)
  {
    int ret;
    ret = tps6586x_write(parent, TPS6586X_LEDPWM, 0xFF);  
...
int ftm_test_regulator_disable(struct regulator *rr)
{
  struct regulator_dev *rdev = rr->rdev;
	struct tps6586x_regulator *ri = rdev_get_drvdata(rdev);
	struct device *parent = to_tps6586x_dev(rdev);
  ...
  if(ri->desc.id == TPS6586X_ID_LED_PWM)
  {
    int ret;
    ret = tps6586x_write(parent, TPS6586X_LEDPWM, 0x00);  
...
static int tps6586x_regulator_enable(struct regulator_dev *rdev)
{
  ...
  if(ldo_test_mode)
  {
    if(id == TPS6586X_ID_LDO_0 ||
      id == TPS6586X_ID_LDO_6  ||
      id == TPS6586X_ID_LDO_7  ||
      id == TPS6586X_ID_LDO_8  ||
      id == TPS6586X_ID_GPIO_1 ||
      id == TPS6586X_ID_GPIO_2 ||
      id == TPS6586X_ID_GPIO_3 ||
      id == TPS6586X_ID_GPIO_4 ||
      id == TPS6586X_ID_LED_PWM )
    {
      
      MSG2("%s, %s, in test mode! Bypass!",__func__,rdev->desc->name);
      return 0;
    }
  }
  else
  {
    MSG2("%s, %s",__func__,rdev->desc->name);
  }
  ...
  if(id == TPS6586X_ID_GPIO_2 && gpio_3v3_capchg_inited)
  {
    ...
    uint8_t reg_val;
  	ret = tps6586x_read(parent, ri->enable_reg[0], &reg_val);
    if(ret) { MSG2("%s, read %02X, Fail = %d", __func__, ri->enable_reg[0], ret);  }
    else    { MSG2("%s, read %02X = %02X", __func__, ri->enable_reg[0], reg_val);  }
    if(!!(reg_val & (1 << ri->enable_bit[0])))  
    {
      MSG2("%s, 3V3_CAPCHG, Bypass", __func__);
    }
  ...
  else if(id == TPS6586X_ID_LED_PWM)
  {
    int ret;
    ret = tps6586x_write(parent, TPS6586X_LEDPWM, 0xFF);  
    MSG2("%s, LEDPWM=0xFF, ret = %d",__func__,ret);
    return ret;
  }
...
static int tps6586x_regulator_disable(struct regulator_dev *rdev)
{
  ...
  if(ri->desc.id == TPS6586X_ID_LED_PWM)
  {
    int ret;
    ret = tps6586x_write(parent, TPS6586X_LEDPWM, 0x00);  
    MSG2("%s, LEDPWM=0x00, ret = %d",__func__,ret);
    return ret;
  }
...
static struct regulator_ops tps6586x_regulator_gpio_ops = {
	.is_enabled = tps6586x_regulator_is_enabled,
	.enable = tps6586x_regulator_enable,
	.disable = tps6586x_regulator_disable,
};
#define TPS6586X_GPIO(_id, vdata, vreg, shift, nbits, \
         ereg0, ebit0, ereg1, ebit1)      \
{                                         \
  .desc = {                               \
    .name = "REG-" #_id,                  \
    .ops  = &tps6586x_regulator_gpio_ops, \
    .type = REGULATOR_VOLTAGE,            \
    .id = TPS6586X_ID_##_id,              \
    .owner  = THIS_MODULE,                \
  },                                      \
  .enable_reg[0]  = TPS6586X_##ereg0,     \
  .enable_bit[0]  = (ebit0),              \
  .enable_reg[1]  = TPS6586X_##ereg1,     \
  .enable_bit[1]  = (ebit1),              \
}
...
static struct tps6586x_regulator tps6586x_regulator[] = {
	...
	TPS6586X_GPIO(GPIO_1, gpio, GPIOSET1, 0, 2, GPIOSET2, 0, GPIOSET2, 0),
	TPS6586X_GPIO(GPIO_2, gpio, GPIOSET1, 2, 2, GPIOSET2, 1, GPIOSET2, 1),
	TPS6586X_GPIO(GPIO_3, gpio, GPIOSET1, 4, 2, GPIOSET2, 2, GPIOSET2, 2),
	TPS6586X_GPIO(GPIO_4, gpio, GPIOSET1, 6, 2, GPIOSET2, 3, GPIOSET2, 3),
	TPS6586X_GPIO(LED_PWM,gpio, LEDPWM,   0, 8, LEDPWM,   0, LEDPWM,   0),
					     ...
					     struct tps6586x_regulator *ri)
{
  ...
  if(ri->desc.id == TPS6586X_ID_GPIO_2)
  {
    if(!gpio_3v3_capchg_inited)
    {
      unsigned gpio = TEGRA_GPIO_PS7; 
      int error = 0;
      ret = gpio_request(gpio, "3V3_CAPCHG");
      if(ret) { MSG2("%s, 3V3_CAPCHG request Fail = %d", __func__, ret);    error ++; }
      else    { MSG2("%s, 3V3_CAPCHG request Pass", __func__); }
      ret = gpio_direction_output(gpio, 0);   
      if(ret) { MSG2("%s, 3V3_CAPCHG output 0, Fail = %d", __func__, ret);  error ++; }
      else    { MSG2("%s, 3V3_CAPCHG output 0, Pass", __func__);  }
      if(!error)
        gpio_3v3_capchg_inited = 1;
    }
