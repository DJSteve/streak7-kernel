...
const char debug_serial_number[] = "0123456789ABCDEF";
u16 debugging_serial;
static int android_set_debug_serial(const char *val, struct kernel_param *kp);
static int android_get_debug_serial(char *buffer, struct kernel_param *kp);
module_param_call(debugging_serial, android_set_debug_serial, android_get_debug_serial,
					&debugging_serial, 0664);
MODULE_PARM_DESC(product_id, "USB debug serial");
...
static int android_set_debug_serial(const char *val, struct kernel_param *kp)
{
	...
	debugging_serial = tmp;

    if (tmp == 0x9443)
    {
        printk("%s use debugging_serial as usb serial number\n", __func__);
        strings_dev[STRING_SERIAL_IDX].s = debug_serial_number;
    }
out:
	return ret;
}
...
static int android_get_debug_serial(char *buffer, struct kernel_param *kp)
{
	...
	ret = sprintf(buffer, "%x", debugging_serial);
...
static int android_probe(struct platform_device *pdev)
{
	...
	if (pdata) {
		...
		if ((pdata->serial_number) && (debugging_serial != 0x9443))
...
static int __init init(void)
{
	...
	printk(KERN_INFO "BootLog +%s+, android init\n", __func__);
