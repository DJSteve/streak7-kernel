...
#ifdef CONFIG_USB_TEGRA_REMOTE_WAKEUP_WORKAROUND
atomic_t tegra_ehci3_wakeup_cnt;

static irqreturn_t tegra_ehci3_wakeup_irq (int irq, void *data)
{
	printk("%s!\n", __func__);
	return IRQ_HANDLED;
}
...
#ifdef CONFIG_USB_TEGRA_REMOTE_WAKEUP_WORKAROUND
	if (!error_status) {
		if ((tegra->wakeup_irq > 0) && (atomic_read(&tegra_ehci3_wakeup_cnt) > 0)) {
			printk("%s: enable remote wakeup gpio\n", __func__);
			enable_irq(tegra->wakeup_irq);
			enable_irq_wake(tegra->wakeup_irq);
		}
	}
#endif
...
#ifdef CONFIG_USB_TEGRA_REMOTE_WAKEUP_WORKAROUND
	if ((tegra->wakeup_irq > 0) && (atomic_read(&tegra_ehci3_wakeup_cnt) > 0)) {
		printk("%s: disable remote wakeup gpio\n", __func__);
		disable_irq_wake(tegra->wakeup_irq);
		disable_irq(tegra->wakeup_irq);
	}
#endif
...
#ifdef CONFIG_USB_TEGRA_REMOTE_WAKEUP_WORKAROUND
	atomic_set(&tegra_ehci3_wakeup_cnt, 0);
	if (instance == 2) {
		int wakeup_gpio = pdata->remote_wakeup_gpio;

		tegra->wakeup_irq = gpio_to_irq(wakeup_gpio);
		if (!tegra->wakeup_irq) {
		...
		tegra_gpio_enable(wakeup_gpio);
		err = gpio_request(wakeup_gpio, hcd->self.bus_name);
		if (err) {
			dev_err(&pdev->dev, "Failed to request wakeup gpio\n");
			goto fail_gpio;
		}
		gpio_direction_input(wakeup_gpio);
		gpio_export(wakeup_gpio, false);

		err = request_irq(tegra->wakeup_irq, tegra_ehci3_wakeup_irq,
				IRQF_TRIGGER_FALLING, pdev->name, pdev);
		if (err) {
			dev_err(&pdev->dev, "Failed to request wakeup IRQ\n");
			goto fail_irq;
		}
		set_irq_type(tegra->wakeup_irq, IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING);
		disable_irq(tegra->wakeup_irq);
	}
#endif
	return err;
...
#ifdef CONFIG_USB_TEGRA_REMOTE_WAKEUP_WORKAROUND
fail_irq:
	gpio_free(pdata->remote_wakeup_gpio);
fail_gpio:
	tegra->wakeup_irq = 0;
#endif
fail:
