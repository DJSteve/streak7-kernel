...
extern atomic_t tegra_ehci3_wakeup_cnt;
#endif
	...
	if (status) {
		...
		if (status != -ESHUTDOWN) {
			...
			if (err && err != -EPERM)
				...
				hw_wwan_suspend_check(port->serial->dev);
			}
		}
...
#ifdef CONFIG_USB_TEGRA_REMOTE_WAKEUP_WORKAROUND
	atomic_inc(&tegra_ehci3_wakeup_cnt);
#endif
	return 0;
...
#ifdef CONFIG_USB_TEGRA_REMOTE_WAKEUP_WORKAROUND
	atomic_dec(&tegra_ehci3_wakeup_cnt);
#endif
	hw_wwan_disconnect(serial);
}
