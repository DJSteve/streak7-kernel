...
#include <sdiovar.h>
    ...
    extern void *addr_4329_bus_rxbuf;
    extern void *addr_4329_bus_databuf;
#endif
...
dhdsdio_probe_malloc(dhd_bus_t *bus, osl_t *osh, void *sdh)
{
	...
	if (bus->dhd->maxctl) {
        bus->rxblen = ROUNDUP((bus->dhd->maxctl + SDPCM_HDRLEN), ALIGNMENT) + DHD_SDALIGN;
#ifdef CONFIG_WLAN_ALLOC_STATIC_MEM
        if (addr_4329_bus_rxbuf!=NULL && (bus->rxblen)<=BUS_RXBUF_LEN)  {
           bus->rxbuf = (uint8 *)addr_4329_bus_rxbuf;
...
#endif
        {
            DHD_ERROR(("%s: static alloc of %d-byte rxbuf failed, addr_4329_bus_rxbuf=0x%p,BUS_RXBUF_LEN=%d\n",
                    __FUNCTION__,bus->rxblen,addr_4329_bus_rxbuf,BUS_RXBUF_LEN));
            if (!(bus->rxbuf = MALLOC(osh, bus->rxblen))) {
                DHD_ERROR(("%s: MALLOC of %d-byte rxbuf failed\n",
                           __FUNCTION__, bus->rxblen));
                goto fail;
...
#ifdef CONFIG_WLAN_ALLOC_STATIC_MEM
    if (addr_4329_bus_databuf!=NULL && MAX_DATA_BUF<=BUS_DATABUF_LEN)
    {
        bus->databuf = (uint8 *)addr_4329_bus_databuf;
...
#endif
    {
        DHD_ERROR(("%s: static alloc of %d-byte rxbuf failed, addr_4329_bus_databuf=0x%p,BUS_DATABUF_LEN=%d\n",
                __FUNCTION__,MAX_DATA_BUF,addr_4329_bus_rxbuf,BUS_DATABUF_LEN));
        if (!(bus->databuf = MALLOC(osh, MAX_DATA_BUF))) {
            DHD_ERROR(("%s: MALLOC of %d-byte databuf failed\n",
                __FUNCTION__, MAX_DATA_BUF));
            
            if (!bus->rxblen && (addr_4329_bus_rxbuf==NULL || (bus->rxblen)>BUS_RXBUF_LEN) )
